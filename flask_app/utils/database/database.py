            ...
            for position in positions:
                ...
                for experience in experiences:
                    ...
                    for skill in skills:
                        ...
                        del skill["skill_id"]
                        del skill["experience_id"]
                        experience["skills"][skill_id] = skill
                    del experience["experience_id"]
                    del experience["position_id"]
                    position["experiences"][experience_id] = experience
                del position["position_id"]
                del position["inst_id"]
                institution["positions"][position_id] = position
            del institution['inst_id']
            sql_dict[inst_id] = institution
        return sql_dict
...
#######################################################################################
    ...
    def createUser(self, email='me@email.com', password='password', role='user'):
        emails = self.query(f"SELECT * FROM users WHERE email = '{email}'")
        if emails:
            return {'success': 0}
        encrypt_password = self.onewayEncrypt(password)
        self.query(f"INSERT INTO users (role, email, password) VALUES ('{role}', '{email}', '{encrypt_password}')")
        return {'success': 1}

    def authenticate(self, email='me@email.com', password='password'):
        encrypt_password = self.onewayEncrypt(password)
        in_database = self.query(f"SELECT * FROM users WHERE email ='{email}' and password = '{encrypt_password}'")
        if in_database:
            return {'success': 1}
        return {'success': 0}
